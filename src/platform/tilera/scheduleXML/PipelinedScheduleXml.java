...
public class PipelinedScheduleXml
{
	 ...
	 * 
	 * TODO : Use Platform xml instead of this.
	 */
	private final int totalProcessorsInPlatform = 62;
	/**
	...
	{		
		...
		for(int i=0;i<totalProcessorsInPlatform;i++)
		{
			...
			{
				procUsedCount++;
				generateProcSchedule (doc, i, procSchedule, scheduleElement);
			}
		}
		
		scheduleElement.setAttribute ("size", Integer.toString(procUsedCount));
	}
	 ...
	 * @param doc XML document 
	 * @param parent parent node where to add this element
	 * @param schedule pipelined schedule model 
	 */
	private void addPreScheduleElement (Graph graph, Solutions solutions, Map<String, String> schedule, Document doc, Element parent)
	{
		...
		if(kMax > 1)
		{
			...
			for(int proc=0;proc<totalProcessorsInPlatform;proc++)
			{
				Element processorElement = doc.createElement ("processor");
				processorElement.setAttribute ("id", Integer.toString(proc));
				int procSchedSize = 0;				
				...
				if(procSchedSize > 0)
				{
					scheduleElement.appendChild(processorElement);
					procAddedSchedCount++;
				}
			}
			if(procAddedSchedCount > 0)
			{
				parent.appendChild(scheduleElement);
				scheduleElement.setAttribute ("size", Integer.toString(procAddedSchedCount));
			}
		}
	}
